...
cp -i -f repos/cosmos-sdk/proto/cosmos "$COSMOS_SDK_DIR/proto/cosmos" || true
...
if [ -d "repos/osmosis/.git" ]; then
  ...
  git -C repos/osmosis checkout master || git -C osmosis checkout main
  git -C repos/osmosis pull origin master || git -C osmosis pull origin main
else
...
cp -i -f repos/osmosis/proto/osmosis "$COSMOS_SDK_DIR/proto/osmosis" || true
...
cp -i -f repos/ibc-go/proto/* "$COSMOS_SDK_DIR/proto/" || true


# WASMD PROTOS

# Extract the Wasmd version from the go.mod file
WASMD_VERSION=$(awk '/github.com\/osmosis-labs\/wasmd/ {print $4}' osmosis/go.mod)
...
cp -i -f repos/wasmd/proto/cosmwasm/wasm/v1/* "$COSMOS_SDK_DIR/proto/cosmwasm/" || true
